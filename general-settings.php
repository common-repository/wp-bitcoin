...
function wp_bitcoin_display_general_settings_menu()
{
    ...
    if(isset($_POST['wp_bitcoin_settings_update'])){
        ...
        $config->setValue('wpbc_bitpay_api_key',trim(sanitize_text_field($_POST["wpbc_bitpay_api_key"])));
        $config->setValue('wpbc_bitpay_transaction_speed',trim(sanitize_text_field($_POST["wpbc_bitpay_transaction_speed"])));
        $config->setValue('wpbc_bitpay_form_page_url',trim(sanitize_text_field($_POST["wpbc_bitpay_form_page_url"])));
        $config->setValue('wpbc_bitpay_form_page_title',trim(sanitize_text_field($_POST["wpbc_bitpay_form_page_title"])));
...
?>
    ...
    <div class="postbox">
    <h3 class="hndle"><label for="title">Usage Instruction</label></h3>
    <div class="inside">
    ...
    <p>Check the <a href="https://www.tipsandtricks-hq.com/wordpress-bitcoin-payment-plugin" target="_blank">WP Bitcoin plugin page</a> for more information</p>
    </div></div>

    <div class="postbox">
    <h3 class="hndle"><label for="title">Bitpay Settings</label></h3>
    <div class="inside">
    ...
    <input name="wpbc_bitpay_api_key" type="text" size="80" value="<?php echo esc_attr($config->getValue('wpbc_bitpay_api_key')); ?>"/>
    ...
    <input name="wpbc_bitpay_form_page_url" type="text" size="80" value="<?php echo esc_url($config->getValue('wpbc_bitpay_form_page_url')); ?>"/>
    ...
    <input name="wpbc_bitpay_form_page_title" type="text" size="80" value="<?php echo esc_attr($config->getValue('wpbc_bitpay_form_page_title')); ?>"/>
    ...
    <p>If you need a feature rich plugin to sell your digital and physical products using bitcoin then checkout our <a target="_blank" href="https://www.tipsandtricks-hq.com/wordpress-estore-plugin-complete-solution-to-sell-digital-products-from-your-wordpress-blog-securely-1059/?utm_source=wp_bitcoin&utm_medium=dashboard&utm_campaign=slogan">WP eStore Plugin</a>
    </p>
    </div>
